...
describe('Forgot passowrd',()=>{
    it('Verify it can send mail when user enter correct format mail ',()=>{
        cy.request({
            ...
            expect(res.body.statusText).to.eq('Your request has been processed. Please check email to reset your password.')
    ...
    it('Verify receive error when user enter mail over 50 character',()=>{
        cy.request({
            ...
            expect(res.status).to.eq(400)
            expect(res.body.statusText).to.eq('One or more validation errors occurred')
            expect(res.body.data[0]).to.eq('The field Email must be a string with a maximum length of 50.')
        })
    })
